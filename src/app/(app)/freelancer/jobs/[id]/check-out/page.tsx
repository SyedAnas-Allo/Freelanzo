...
export default async function FreelancerCheckOutPage({
  params,
  searchParams,
}: {
  ...
  if (!hasCheckIn) {
    redirect(`/freelancer/jobs/${id}/check-in?date=${workDate}`);
  }

  const alreadyDone = (events ?? []).some(
    (e) => e.kind === "check_out" && e.work_date === workDate,
  );

  const recordedEvent = alreadyDone
    ? await loadAttendanceRecordView(supabase, app.id, "check_out", workDate)
    : null;
